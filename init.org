...
* Final touches
This Emacs configuration sets up lots of packages and configures a number of keybindings. To add our own customizations, place them in =~/.emacs.d/custom.el=. This file is tracked in the [[https://github.com/IQSS/IQSS.emacs][IQSS.emacs]] git repository (see the README for documented settings: frame size, Magit, Copilot, etc.) and is sourced last, so you always have the ability to override any settings provided here.
