...
** How to I install it?
1) Make sure you have [[http://git-scm.com/downloads][git]] installed on your computer.
2) Back up your existing =~/.emacs= file and =~/.emacs.d= directory (e.g., rename =.emacs= to =OLD.emacs= and rename =.emacs.d= to =OLD.emacs.d=).
3) Clone one of the following versions into =~.emacs.d=:
   - master :: Relatively conservative, probably the one you want.
   - Gary :: Like master but with large font by default.
   - izahn :: Master plus more experimental packages at settings. Use
              at your own risk! 

For example, to use Gary's configuration open a terminal and run =git clone https://github.com/izahn/dotemacs.git ~/.emacs.d=
