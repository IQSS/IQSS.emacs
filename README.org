...
:END:
- M-/ :: Mapped to =company-complete=, use for completion menu.
- C-M-/ :: Mapped to =counsel-company=, use for search-able completion menu.
...
:END:
Here we configure in-buffer text completion using the company-mode package. These completions are available on-demand using the =M-/= or =C-M-/=.
