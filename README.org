...
:END:
1) Make sure emacs >= version 25.1 is installed on your computer. See [[#suggested-external-programs][Suggested-external-programs]] for installation instructions.
...
** Composable key bindings
Emacs keybindings are not really composable (or really even consistent) by default. The =composable= package attempts to rectify that by bringing vim-style composability in a way that is consistent with standard Emacs keybindings. More information is available at https://github.com/paldepind/composable.el.
...
Emacs comes with spell checking built-in, it just needs to be turned on. By default automatic spell checking is enabled in =text-mode= and =prog-mode= buffers. More information is available at https://www.gnu.org/software/emacs/manual/html_mono/flymake.html#Using-Flymake.
