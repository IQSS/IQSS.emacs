...
7) Clone this repository into =~/.emacs.d= by opening a terminal and running =git clone http://github.com/izahn/dotemacs ~/.emacs.d=.

If you don't know how to use git, you can skip steps 2 and 7 and simply [[https://github.com/izahn/dotemacs/archive/master.zip][download the files as a zip archive]], extract them, and move them into your .emacs.d directory instead.
...
The basic window navigation with =C-c C-o= does not make it easy to navigate quickly to a specific window, so we use windmove for that. Use =M-S-left/right/up/down= to quickly navigate to a window.
...
Emacs comes with spell checking built-in, it just needs to be turned on. By default spell checking is enabled in =text-mode= and =prog-mode= buffers.
...
We used to do very complicated stuff to get auto completion in Emacs. I no longer believe it's worth it; the default completion is good enough. If you need completions just press =tab=. If it doesn't work, press =tab= again. That's all.
