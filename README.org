...
Many programming language modes derive from =prog-mode=, so we can configure all of them by changing =prog-mode= settings. Here we turn on =outline-mode= and set up =completion-at-point= functions for programming modes.
...
Emacs makes a decent python IDE thanks to https://github.com/jorgenschaefer/elpy. As with other programming languages you can get completion suggestions with the =tab= key, and evaluate code with =C-ret=. Many more features are provided by =elpy=, refer to https://elpy.readthedocs.io/en/latest/index.html for details.
...
*** Typesetting markup (AucTeX)
    ...
    :END:
I don't write nearly as much in LaTeX as I used to, as Markdown and/or Org mode are simpler and good enough for my needs. But LaTeX is still the tool of choice for much academic writing, so we use AUCTEX and turn on lots of features. See https://www.gnu.org/software/auctex/ for details.
